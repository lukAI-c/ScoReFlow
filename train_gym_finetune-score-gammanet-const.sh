...
#
# 支持任务:
#   humanoid  → FlowMLP  + PPOFlowWithScoreConst  (ppoflow_with_score_gammanet_const)
#   kitchen   → ShortCut + PPOShortCutWithScoreConst (pposhortcut_with_score_gammanet_const)
#
# 消融对比:
...
#   TASK=kitchen  ALPHA=0.0 bash train_gym_finetune-score-gammanet-const.sh
#   TASK=kitchen  ALPHA=1.0 bash train_gym_finetune-score-gammanet-const.sh
...
if [ "${TASK}" = "humanoid" ]; then
    MUJOCO_GL="egl" xvfb-run -a -s "-screen 0 1024x768x24" python script/run.py \
        ...
        --config-name=ft_ppo_reflow_mlp_with_score_gammanet_const \
...
# ── Kitchen-complete ──────────────────────────────────────────────────
    ...
    MUJOCO_GL="egl" xvfb-run -a -s "-screen 0 1024x768x24" python script/run.py \
        ...
        --config-name=ft_ppo_shortcut_mlp_with_score_gammanet_const \
