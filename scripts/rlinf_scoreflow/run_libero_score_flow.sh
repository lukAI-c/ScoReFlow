...
# Minimal RLinf/OpenPI LIBERO runner for original Score-Flow.
...
MODEL_DIR="${MODEL_DIR:-/path/to/RLinf-Pi0-LIBERO-Spatial-Object-Goal-SFT}"
EXP_ROOT="${EXP_ROOT:-${RLINF_ROOT}/logs/scoreflow_original_libero}"

LIBERO_POLICY_VARIANT="${LIBERO_POLICY_VARIANT:-pi0}"
...
config_for_suite() {
  if [[ "${LIBERO_POLICY_VARIANT}" == "pi05" || "${LIBERO_POLICY_VARIANT}" == "pi0.5" ]]; then
    case "$1" in
    ...
    case "$1" in
      libero_spatial) echo "libero_spatial_ppo_openpi_quickstart" ;;
...
method_overrides() {
  case "$1" in
    flow_noise_baseline)
      echo "actor.model.openpi.noise_method=flow_noise ++actor.model.openpi.score_flow_mode=none ++actor.model.openpi.score_flow_scale=0.0"
      ;;
    scoreflow_original)
      echo "actor.model.openpi.noise_method=flow_noise ++actor.model.openpi.score_flow_mode=learned_alpha ++actor.model.openpi.score_flow_scale=1.0 ++actor.model.openpi.score_flow_clip_norm=10.0 ++actor.model.openpi.score_flow_alpha_hidden_dim=16 ++actor.model.openpi.score_flow_alpha_init_bias=-2.0 ++actor.model.openpi.score_flow_alpha_max=2.0 ++actor.model.openpi.score_flow_use_time_mask=true"
      ;;
    *)
...
append_manifest_header() {
  if [[ ! -f "${MANIFEST}" ]]; then
    echo "suite,method,seed,run_name,status,run_dir,start_time,end_time,exit_code" > "${MANIFEST}"
  fi
}
...
run_one() {
  ...
  local method="$2"
  local seed="$3"
  local config_name run_name run_dir run_log start_time end_time exit_code status
  ...
  printf "%s,%s,%s,%s,%s,%s,%s,%s,%s\n" \
    "${suite}" "${method}" "${seed}" "${run_name}" "${status}" "${run_dir}" \
