...
#
# 用法:
#   bash train_robomimic_finetune-score-alphanet-const.sh            # 默认 alpha=0 (cfg 默认值)
#   ALPHA=0.0 bash train_robomimic_finetune-score-alphanet-const.sh  # 显式 alpha=0
#   ALPHA=1.0 bash train_robomimic_finetune-score-alphanet-const.sh  # alpha=1
...
python run.py \
    ...
    --config-name=ft_ppo_shortcut_mlp_img_with_score_alphanet_const \
