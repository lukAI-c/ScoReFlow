...
# Franka Kitchen Fine-tuning Script (Base)
...
MUJOCO_GL="egl" xvfb-run -a -s "-screen 0 1024x768x24" python script/run.py \
    ...
    base_policy_path=${REINFLOW_LOG_DIR}/gym/pretrain/kitchen-complete-v0_pre_reflow_mlp_ta4_td20_seed42/checkpoint/best.pt \
    device=cuda:0 \
    sim_device=cuda:0 \
