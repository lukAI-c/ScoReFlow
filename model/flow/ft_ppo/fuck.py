...
class EvalDualStreamAgent(EvalAgent):
    ...
    def load_model_for_eval(self):
        ...
        if self.load_ema and 'ema' in data:
            ...
            # 检查是否是完整的 DualStream checkpoint
            has_dual_stream = any(k.startswith('actor_prior.') or k.startswith('actor_reward.') 
            ...
            if has_dual_stream:
                ...
                if missing:
                    log.warning(f"Missing keys: {missing[:3]}... (total {len(missing)})")
                if unexpected:
                    log.warning(f"Unexpected keys: {unexpected[:3]}... (total {len(unexpected)})")
            else:
