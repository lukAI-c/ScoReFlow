...
# PPOFlow with Score-guided drift + Constant Alpha (Ablation)

"""
PPOFlow + Score 导向 + 固定常数 alpha（消融实验用，Flow 版本）

继承自 PPOFlowWithScore，仅覆盖 get_alpha_t：
    - alpha_constant=0.0  → α(t)≡0，退化为纯 NoisyFlow（无 score）
    - alpha_constant=1.0  → α(t)≡1，score 全程以常数强度作用（不随 t 衰减）
    - alpha_constant=None → 回退到父类行为（time_mask * gamma_score 或 GammaNet）

适用任务：Humanoid、Ant、Walker2d 等使用 FlowMLP 的 gym 任务。
ShortCut 任务（Kitchen）请使用 pposhortcut_with_score_gammanet_const.py。
...
cfg 用法:
    model:
      _target_: model.flow.ft_ppo.ppoflow_with_score_gammanet_const.PPOFlowWithScoreConst
      alpha_constant: 0.0   # 或 1.0
      score_scheduler_type: fixed
      ...其余参数与 ppoflow_with_score_gammanet 完全相同
"""
...
class PPOFlowWithScoreConst(PPOFlowWithScore):
    """
    消融版本（Flow）：alpha(t) 为用户指定的定值，不乘 time_mask。

    支持两种典型消融：
        alpha_constant=0.0  → 完全关闭 score 引导，等价于纯 NoisyFlow
        alpha_constant=1.0  → score 以常数强度作用，验证 time_mask 的必要性

    当 alpha_constant=None 时行为与父类 PPOFlowWithScore 完全一致。
    """
    ...
    def __init__(self,
        ...
        if alpha_constant is not None:
            logging.info(
                f"PPOFlowWithScoreConst: alpha_constant={alpha_constant} "
                f"(time_mask DISABLED, score runs at constant α)"
            )
        else:
            logging.info("PPOFlowWithScoreConst: alpha_constant=None, fallback to parent get_alpha_t")

    
    # def loss(self, *args, **kwargs):
    #     result = super().loss(*args, **kwargs)
    #     # 父类返回 16 个值，最后一个是 alpha_curve_dict，去掉它返回 15 个
    #     return result[:-1]
    # ------------------------------------------------------------------
    # 唯一覆盖：get_alpha_t
    # ------------------------------------------------------------------
