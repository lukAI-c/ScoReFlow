...
class PPOShortCutWithScoreGammaNetObs(nn.Module, ScoreFunctionMixin):
    ...
    def get_alpha_t(self, t: Tensor, cond_emb: Tensor = None) -> Tensor:
        """
        获取时间依赖的 Score 系数 alpha_t (带 Time Decay Mask + Observation-Dependent)

        Args:
            t: [B] 时间步 ∈ [0, 1]
            cond_emb: [B, cond_enc_dim] 条件嵌入（从 actor_ft.forward 获取）
        Returns:
        ...
        if self.score_scheduler is None:
            ...
            # 使用传入的 cond_emb（从 actor_ft.forward 获取）
            if cond_emb is None:
                # 如果没有传入 cond_emb，用零向量，并报警告
                cond_emb = torch.zeros(B, self.cond_enc_dim, device=self.device)
                logging.warning(
                    "get_alpha_t: cond_emb is None. "
                    "Using zero vector for cond_emb. "
                    "Expected cond_emb with shape [B, cond_enc_dim]."
                )
    ...
    def get_actions(self,
        ...
        # ========== 预计算 cond_emb (Observation-Dependent) ==========
        # 由于 cond 在整个采样过程中不变，我们只需要计算一次 cond_emb
        # 这样可以避免在每次迭代中重复计算，提高效率
        with torch.no_grad():
            # 使用一个 dummy action 和 time 来获取 cond_emb
            dummy_action = torch.zeros((B, self.horizon_steps, self.action_dim), device=self.device)
            dummy_t = torch.zeros(B, device=self.device)
            dummy_d = torch.full((B,), dt, device=self.device)
            _, _, cond_emb = self.actor_ft.policy.forward(
                dummy_action, dummy_t, dummy_d, cond, output_embedding=True
            )
        ...
        for i in range(self.inference_steps):
            ...
            alpha_t = self.get_alpha_t(t, cond_emb)  # [B, 1, 1]  ← 传入 cond_emb
