...
class PPOFlowWithScore(nn.Module, ScoreFunctionMixin):
    ...
    def _init_score_scheduler(self, hidden_dim: int):
        """
        初始化自适应 Score Scheduler

        输入: t (时间) + condition (obs, 用于自适应)
        ...
        """
        # 输入维度: 1 (时间) + obs_dim (条件)
        # 如果 obs 太大，建议先用一个 Linear 降维
        input_dim = 1 + self.obs_dim
    ...
    def get_actions(self,
        ...
        for i in range(self.inference_steps):
            ...
            # ========== 核心修改：使用时间调度的 Score 强度 ==========
            st = self.compute_score(xt, vt, t)

            # 获取时间依赖的 alpha_t
            alpha_t = self.get_alpha_t(t, cond)  # [B, 1, 1]

            # drift = vt + alpha_t * std * st
            # alpha_t 由 score_scheduler 根据时间 t 动态调整
            # std 是学习到的噪声标准差
            drift = vt + alpha_t * std * st
    ...
    def get_logprobs(self,
        ...
        for i in range(self.inference_steps):
            ...
            # 处理噪声标准差 (nt 形状可能是 [B, Ta*Da] 或 [B, Ta, Da])
            if nt.dim() == 2:
                nt_expanded = nt.reshape(B, self.horizon_steps, self.action_dim)
            else:
                nt_expanded = nt
            ...
            alpha_t = self.get_alpha_t(t, cond)  # [B, 1, 1]
            # 将 tensor 转为标量以方便查看
            # st_norm = st.norm().item()
            # vt_norm = vt.norm().item()
            # alpha_val = alpha_t.flatten()[0].item()

            # 修改 print 行如下：
            # print(f"[Step {int(t):3d}] alpha: {float(alpha_val):.4f} | score_norm: {float(st_norm):.4f} | vel_norm: {float(vt_norm):.4f}")
            # drift = vt + alpha_t * nt * st
            drift = vt + alpha_t * nt_expanded * st
