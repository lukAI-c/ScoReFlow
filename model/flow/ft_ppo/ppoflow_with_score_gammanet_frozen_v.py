...
# PPOFlow with Score-guided drift - Frozen Velocity Field variant
# 冻结速度场 v_t，只学习 variance (noise net) + score scheduler (alpha_t)

"""
Frozen-V variant of PPOFlowWithScore + GammaNet

与 ppoflow_with_score_gammanet.py 的唯一区别:
    速度场 v_t 完全冻结，梯度只流向:
...
动机:
    预训练的速度场已经足够好，冻结它可以:
    - 大幅减少可训练参数量
    - 避免 RL 微调破坏预训练的 flow matching 质量
    - 让优化聚焦在探索策略 (noise) 和 score 调度上
"""
...
class PPOFlowWithScoreFrozenV(nn.Module, ScoreFunctionMixin):
    """
    PPOFlow + Score + GammaNet, with frozen velocity field.
    ...
    def __init__(self,
        ...
        # 创建可训练副本 (NoisyFlowMLP 需要 policy 做前向, 先全部设 requires_grad=True)
    ...
    def get_alpha_t(self, t: Tensor) -> Tensor:
        ...
        alpha_constant 不为 None 时: 直接返回固定常数，忽略 time_mask 和 scheduler
            alpha_constant=0.0 → 关闭 score 引导（纯 NoisyFlow）
    ...
    def report_network_params(self):
        ...
        logging.info(
            f"PPOFlowWithScoreFrozenV - Network params:\n"
