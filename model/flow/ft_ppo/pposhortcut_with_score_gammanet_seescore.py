...
class PPOShortCutWithScore(nn.Module, ScoreFunctionMixin):
    ...
    def _init_score_scheduler(self, hidden_dim: int):
        ...
        if not self.use_score_scheduler or self.score_scheduler_type == 'fixed':
            ...
            nn.init.constant_(self.score_scheduler[-2].weight, 0.1) # 初值0.1
            nn.init.constant_(self.score_scheduler[-2].bias, 0.0) # 初值0
    ...
    def get_alpha_t(self, t: Tensor) -> Tensor:
        ...
        # 使用 (1-t) 作为 mask
        time_mask = (t < 0.7).float().view(B, 1)  # [B, 1], t >= 0.75 时为 0
    ...
    def get_logprobs(self,
        ...
        # 用于收集 score 相关统计信息
        alpha_t_list = []   # alpha_t (score scheduler 输出)
        alpha_st_list = []  # alpha_t * st
        st_list = []        # score (st)
        nt_list = []        # 噪声网络输出 (noise std)
        # Initial probability
        ...
        for i in range(self.inference_steps):
            ...
            alpha_st = alpha_t * st  # 保存用于统计
            drift = vt + alpha_st

            # 收集统计信息 (detach 避免影响梯度计算)
            alpha_t_list.append(alpha_t.detach())
            alpha_st_list.append(alpha_st.detach())
            st_list.append(st.detach())
            nt_list.append(nt.detach())
        ...
        # 计算 score 相关统计信息
        if alpha_st_list:
            alpha_t_all = torch.stack(alpha_t_list, dim=0)    # [T, B, 1] alpha_t (score scheduler 输出)
            alpha_st_all = torch.stack(alpha_st_list, dim=0)  # [T, B, H, A]
            st_all = torch.stack(st_list, dim=0)              # [T, B, H, A]
            nt_all = torch.stack(nt_list, dim=0)
            score_stats = {
                'alpha_t_mean': alpha_t_all.mean().item(),    # score scheduler 输出均值
                'alpha_t_std': alpha_t_all.std().item(),      # score scheduler 输出标准差
                'alpha_st_mean': alpha_st_all.mean().item(),
                'alpha_st_std': alpha_st_all.std().item(),
                'st_mean': st_all.mean().item(),
                'st_std': st_all.std().item(),
                'nt_mean': nt_all.mean().item(),  # 噪声网络输出均值
                'nt_std': nt_all.std().item(),    # 噪声网络输出标准差
            }
        else:
            score_stats = {                
                'alpha_t_mean': 0.0,
        ...
        if get_entropy:
            if get_chains_stds:
                return logprob, entropy_rate_est, chains_stds.mean(), score_stats
            return logprob, entropy_rate_est, score_stats
        else:
            if get_chains_stds:
                return logprob, chains_stds.mean(), score_stats
            return logprob, score_stats
    ...
    def loss(
        ...
        """
        newlogprobs, entropy, noise_std, score_stats = self.get_logprobs(
            obs,
            chains,
            get_entropy=True,
        ...
        return (
            ...
            noise_std.item(),
            newvalues.mean().item(),
            score_stats,  # 包含 alpha_st_mean, alpha_st_std, st_mean, st_std
        )
