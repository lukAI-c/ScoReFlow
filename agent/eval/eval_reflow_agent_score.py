# MIT License
# Copyright (c) 2025 ReinFlow Authors - Score-based Evaluation

"""
Evaluate Score-based PPO Flow policy.

基于 Score 函数的 SDE 采样评估：
    SDE: dxt = [bt(xt) + εt·st(xt)] dt + √(2εt) dWt
    其中 st(x) = (t * bt(x) - x) / (1 - t) 为分数函数

评估时可选择：
    - eval_mode=True: 确定性采样 (无噪声)
    - eval_mode=False: 随机采样 (带噪声探索)
"""
import logging
import torch
...
class EvalScoreAgent(EvalAgent):
    """Score-based PPO Flow 评估 Agent"""
    ...
    def __init__(self, cfg):
        ...
        log.info(f"Score Eval: load_ema={self.load_ema}, eval_mode={self.eval_mode}, "
