# MIT License
# Copyright (c) 2025 ReinFlow Authors - ScoRe-Flow Image Evaluation

"""
Evaluate ScoRe-Flow (with_score_gammanet) policy with image inputs.

ScoRe-Flow 采样评估 (图像输入版本):
...
class EvalImgScoreGammaNetAgent(EvalImgAgent):
    """ScoRe-Flow (with_score_gammanet) 图像输入评估 Agent"""
    ...
    def __init__(self, cfg):
        ...
        self.record_video = cfg.get('record_video', cfg.env.get('save_video', False))
        self.record_env_index = 0
        self.frame_width = 640
        self.frame_height = 480
        self.render_onscreen = cfg.get('render_onscreen', False)
        self.denoising_steps = cfg.get("denoising_step_list", [1, 4])
        self.denoising_steps_trained = cfg.get("ft_denoising_steps", 1)
        ...
        self.min_std = cfg.get("min_std", 0.08)
        self.max_std = cfg.get("max_std", 0.14)

        log.info(f"ScoRe-Flow Img Eval: load_ema={self.load_ema}, eval_mode={self.eval_mode}, "
    ...
    def load_model_for_eval(self):
        ...
        支持的 checkpoint 格式：
        1. 完整的 PPOFlowWithScore/PPOShortCutWithScore checkpoint
        2. EMA checkpoint
        """
        ...
        has_score_scheduler = any(k.startswith('score_scheduler.') for k in state_dict.keys())
        has_noise_scheduler = any(k.startswith('noise_scheduler.') for k in state_dict.keys())

        log.info(f"Checkpoint has: actor_ft={has_actor_ft}, "
                 f"score_scheduler={has_score_scheduler}, noise_scheduler={has_noise_scheduler}")

        if has_actor_ft:
            # 完整的 PPOFlow checkpoint - 直接加载
            missing, unexpected = self.model.load_state_dict(state_dict, strict=False)
            log.info(f"Loaded complete ScoRe-Flow checkpoint")
            if missing:
        ...
        log.info(f"Model ScoRe-Flow params loaded successfully")
