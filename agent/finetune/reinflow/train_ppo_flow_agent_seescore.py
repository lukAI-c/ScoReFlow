...
class TrainPPOFlowAgent(TrainPPOAgent):
    ...
    def run(self):
        ...
        while self.itr < self.n_train_itr:
            ...
            # Collect alpha network statistics (if available)
            alpha_log_dict = self._collect_alpha_statistics()

            self.log(train_log_dict_additional=alpha_log_dict)  # diffusion_min_sampling_std
            self.update_lr()
            self.adjust_finetune_schedule()# update finetune scheduler of ReFlow Policy
            self.save_model()
            self.itr += 1
            
    def _collect_alpha_statistics(self):
        """
        收集 alpha 网络的统计信息（如果模型有 get_alpha_t 方法）

        Returns:
            dict: alpha 网络的统计信息，用于 WandB 日志
        """
        alpha_log_dict = {}

        # 检查模型是否有 get_alpha_t 方法（Score Scheduler）
        if not hasattr(self.model, 'get_alpha_t'):
            return alpha_log_dict

        # 检查模型是否有 use_score_scheduler 属性
        if not hasattr(self.model, 'use_score_scheduler') or not self.model.use_score_scheduler:
            return alpha_log_dict
        ...
        try:
            with torch.no_grad():
                ...
                # 采样最多 100 个观测
                B = min(len(self.buffer.obs), 100)
                obs_samples = torch.stack([self.buffer.obs[i] for i in range(B)]).to(self.device)

                # 构造 cond
                cond = {'state': obs_samples}

                # 随机采样时间点
                t = torch.rand(B, device=self.device) * 0.99

                # 计算 alpha_t
                alpha_t = self.model.get_alpha_t(t, cond)  # [B, 1, 1]
                alpha_t = alpha_t.squeeze(-1).squeeze(-1)  # [B]
            ...
            log.warning(f"Failed to collect alpha statistics: {e}")
    ...
    def agent_update(self, verbose=True):
        ...
        for update_epoch, batch_id, minibatch in self.minibatch_generator() if not self.repeat_samples else self.minibatch_generator_repeat():
            ...
            oldlogprob_min, oldlogprob_max, oldlogprob_std, \
                ...
                noise_std, Q_values, score_stats = self.model.loss(*minibatch,
            ...
            # 检查 score_scheduler 的梯度
            scheduler_norm = 0.0
            if hasattr(self.model, 'score_scheduler') and self.model.score_scheduler is not None:
                scheduler_norm = torch.nn.utils.clip_grad_norm_(self.model.score_scheduler.parameters(), max_norm=float('inf'))

            if verbose:
                log.info(f"before clipping: actor_norm={actor_norm:.2e}, critic_norm={critic_norm:.2e}, actor_old_norm={actor_old_norm:.2e}, scheduler_norm={scheduler_norm:.2e}")
        ...
        self.train_ret_dict = {
                ...
                "scheduler_norm": scheduler_norm,  # score_scheduler 梯度范数
                ...
                "Q_values": Q_values,
                # Score 相关统计信息 (alpha_t * st 和 st)
                "alpha_st_mean": score_stats.get('alpha_st_mean', 0.0),
                "alpha_st_std": score_stats.get('alpha_st_std', 0.0),
                "st_mean": score_stats.get('st_mean', 0.0),
                "st_std": score_stats.get('st_std', 0.0),
                # 噪声网络输出统计
                "nt_mean": score_stats.get('nt_mean', 0.0),
                "nt_std": score_stats.get('nt_std', 0.0),
                # Score scheduler 输出 (alpha_t)
                "alpha_t_mean": score_stats.get('alpha_t_mean', 0.0),
                "alpha_t_std": score_stats.get('alpha_t_std', 0.0),
            }
